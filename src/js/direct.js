...
function renderizarConversas(chatsArray, filtrarTermo = "") {
  ...
  for (const chatObj of chatsArray) {
    ...
    btn.innerHTML = `
      <img src="${friendPhotoUrl}" alt="Foto" onerror="this.src='./src/img/default.jpg'" >
...
dmChatUserName.addEventListener("click", () => {
  const username = dmChatUserName.getAttribute("data-userid");
  if (username) window.location.href = `profile.html?u=${username}`;    
});
